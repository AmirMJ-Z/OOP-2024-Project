    ...
    private void initializeNotebook() throws SQLException {
        ...
        for (Card i : cards) {
            if (app.getCurrentUser().getCardLibrary().getCardLevel1ByName(i.getName()) != null) {
                Card card = app.getCurrentUser().getCardLibrary().getCardLevel1ByName(i.getName());
                card = card.updradeCard();
                notebook.addElement(card);
            }
        }
    }

    public UpgradeShopController() throws SQLException {
        initializeNotebook();
    }
...
//        app.setCurrentMenu(mainMenu);
    ...
    public String buy(int num) throws SQLException {
        ...
        if (app.getCurrentUser().getCoin() < card.getPrice()) {
            return "You dont have enough coins to upgrade this card";
        }

        if (app.getCurrentUser().getLevel() < card.getMinLevel()) {
            return "You need to be level " + card.getMinLevel() + " to be able to upgrade this card";
        }

        app.getCurrentUser().getCardLibrary().upgradeCard(card);
        app.getCurrentUser().reduceCoins(card.getPrice());
        initializeNotebook();

        return "Card " + card.getName() + " Level " + card.getLevel() + " Was upgraded successfully.";
    }
}
