...
public class CardDeleteController {
    ...
    public CardDeleteController() throws SQLException {
        initializeNotebook();
    }
...
//        app.setCurrentMenu(adminMenu);
    ...
    public String deleteCard(int index) throws SQLException, IOException, ClassNotFoundException {
        ...
        DeleteConfirmationPopup deleteConfirmationPopup = new DeleteConfirmationPopup();
        if (deleteConfirmationPopup.start()) {
            database.deleteCard(card);
            initializeNotebook();

            return "Card " + card.getName() + " was deleted from the database successfully";
        }

        return "Card delete cancelled";
    }
}
